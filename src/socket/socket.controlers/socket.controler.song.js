...
export async function addSong(io, socket, data) {
    try {
        const { title, thumbnail, songId, position } = data
        const addedBy = socket.data.mongoId
        const sessionId = socket.data.sessionId
        if (!title || !thumbnail || !songId || !position || !sessionId || !addedBy) {
            const error = new Error("Required params not given")
            error.status = 400
            throw error;
        }
        
        const serviceCall = await songService.addSong(title, thumbnail, songId, position, addedBy, sessionId);
