...
import { getMyGroups } from '../../services/group.service/group.service.getMyGroups.js';
...
export async function groupCreateControler(req, res) {
    try {
        ...
        if (!name || !mod || typeof isPrivate !== "boolean") {
...
export async function groupGetMyGroupsControler(req, res) {
    try {
        ...
        const userId = req.userId;
        const serviceCall = await getMyGroups(userId)
        res.status(200).json({
            success: true,
            message: "Group added succefully",
            data: serviceCall.data
